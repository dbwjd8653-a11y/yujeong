...
@allure.title("[FHC-093] 내 기관 클릭 시 UI 및 정보 표시")
@allure.severity(allure.severity_level.NORMAL)
def test_org_info_displayed(mypage):
    """
    [FHC-093] 내 기관 클릭 시 UI 및 정보 표시
    ...
    """
    with allure.step("[FHC-093] 내 기관 페이지 이동"):
        mypage.navigate_to_org()
    with allure.step("[FHC-093] 내 기관 UI 및 정보 표시 확인"):
...
def test_qaproject_link(mypage):
    ...
    """
    with allure.step("[FHC-094] 내 기관 페이지 이동 후 qaproject 링크 클릭"):
        mypage.navigate_to_org()
    ...
    with allure.step("[FHC-094] 새 탭 생성 확인"):
...
@allure.title("[FHC-095] 헬프 센터 하이퍼링크 작동")
@allure.severity(allure.severity_level.NORMAL)
def test_help_center_link(mypage):
    """
    [FHC-095] 헬프 센터 하이퍼링크 작동
    ...
    """
    with allure.step("[FHC-095] 내 기관 페이지 이동 후 헬프 센터 링크 클릭"):
        mypage.navigate_to_org()
        original_handle  = mypage.driver.current_window_handle
        original_handles = mypage.driver.window_handles[:]
        mypage.click_help_center_link()
    with allure.step("[FHC-095] 새 탭 생성 확인"):
        assert mypage.is_new_tab_opened(original_handles), \
