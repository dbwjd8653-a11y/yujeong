...
| 지표 | 결과 |
|------|------|
| 테스트 케이스 수 (현재) | 고유 **63개** → Edge·Chrome 교차 **실행 126개** (브라우저당 selected 63) |
| 크로스 브라우저 매트릭스 | **Edge · Chrome 병렬 교차 실행** (`fail-fast: false` - 팀 시절 대비 신규 도입) |
| 최근 CI 테스트 통과율 | **스킵 제외 100% (110 / 110 통과)** - 실패 0 · 스킵 16 · 실행 126 (수집 130 중 slow·xfail 4 마커 제외) |
| CI 실행 시간 | 브라우저당 **약 9~12분** (Edge · Chrome 병렬 잡) |
| POM 도입 후 유지보수 효율 | 공통 UI 변경 시 수정 파일 **22개 → 1개**, 약 **95.5% 절감** (로그인 사용 22/24 테스트 파일 기준) |

> 수치 정의 (브라우저당): **수집(collected) 65** → slow·xfail 마커 **2 제외** → **선택 실행(selected) 63** = 통과 55 + 스킵 8. 매트릭스 2종 합산 시 ×2 → 실행 126 · 통과 110 · 스킵 16
>
> 지표 출처: `.github/workflows/test.yml`(매트릭스·파이프라인), GitHub Actions 실행 기록(통과율·시간)
>
> ⏭ **스킵 16건은 모두 환경·서비스 변경에 따른 의도적 스킵이며, 테스트 자체 결함이 아님**
>
> 🔁 Allure 리포트 총계가 회차에 따라 128~129로 보일 수 있으나, 이는 flaky 2건의 자동 재시도(`@pytest.mark.flaky(reruns=2)`) 결과 엔트리가 합산된 값 — 실제 케이스 수는 **126으로 고정**
...
> 💡 실제 자동 생성된 티켓 예시 스크린샷을 `images/`에 추가하면 기능을 더 직관적으로 보여줄 수 있음
...
> UI·로케이터가 바뀌어도 해당 Page 파일 하나만 수정하면 됨
...
**2. headless 토큰 한도 저장 테스트 - 상태 의존 flaky**
MUI '저장' 버튼이 토글 값과 서버 저장값이 다를 때만 활성화되는데, 테스트가 끝 상태만 맞추고 순변경을 보장 못 해 직전 저장 상태에 따라 저장 버튼이 비활성으로 남아 간헐 타임아웃
→ 본 검증 전 반대 상태를 먼저 저장해 baseline 고정(항상 실제 변경 발생). 재현 시 3회 중 2회 실패하던 테스트를 결정적 통과로 안정화 (Chrome·Edge)
