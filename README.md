...
| 지표 | 결과 |
|------|------|
| 테스트 케이스 수 (현재) | 고유 **63개** · Edge·Chrome 교차 **실행 126개** |
| 크로스 브라우저 매트릭스 | **Edge · Chrome 병렬 교차 실행** (`fail-fast: false` - 팀 시절 대비 신규 도입) |
| 최근 CI 테스트 통과율 | **스킵 제외 100% (110/110)** · **스킵 포함 87.3% (110/126)** · 실패 0 |
| CI 실행 시간 | 브라우저당 **약 9~12분** (Edge · Chrome 병렬 잡) |
| POM 도입 후 유지보수 효율 | 공통 UI 변경 시 수정 파일 **22개 → 1개**, 약 **95.5% 절감** (로그인 사용 22/24 테스트 파일 기준) |

> **수치 정의** (브라우저당): 수집 65 → slow·xfail 마커 2 제외 → 선택 실행 63 (통과 55 + 스킵 8). 매트릭스 2종 합산 → **실행 126 · 통과 110 · 스킵 16**
> 출처: `.github/workflows/test.yml`, GitHub Actions 실행 기록
>
> ⏭ 스킵 16건은 모두 환경·서비스 변경에 따른 **의도적 스킵** (테스트 자체 결함 아님)
>
> 🔁 Allure 리포트 총계는 flaky 자동 재시도(`@pytest.mark.flaky`) 엔트리가 합산돼 회차마다 126보다 크게 보일 수 있으나, 실제 케이스 수는 **126 고정**
