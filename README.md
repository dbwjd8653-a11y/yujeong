...
MUI '저장' 버튼은 토글 값이 **서버에 저장된 값과 다를 때만(form dirty)** 활성화되는데, 테스트가 토글을 두 번 눌러 *끝 상태*만 맞추고 *순변경*을 보장하지 않았다. 직전 실행이 남긴 저장값에 따라 변경량이 0이 되면 저장 버튼이 비활성으로 남아 타임아웃 (+ 광범위 `//button[="저장"]`가 숨은 첫 매치를 잡던 문제, headless에서 MUI 토글 JS click이 React onChange로 간헐 미전달되는 문제 동반)
→ 본 검증 전 **반대 상태를 먼저 저장해 baseline을 고정**(항상 실제 변경 발생) + 저장 버튼을 `type="submit"`·노출/활성 요소로 한정 + 토글 최대 3회 재클릭. **재현 시 3회 중 2회 타임아웃으로 실패하던 테스트를 결정적으로 통과**하도록 안정화 (Chrome·Edge)
